...
1. **Assignment Rule**
$$ \frac{}{\{Post[x/e]\}~x := e~\{Post\}} $$

2. **Block Rule**
$$ \frac{\{Pre\}~c~\{Post\}~~Declared(c) \cap \VarsOf{Post} = \emptyset}{\{Pre\}~\text{begin}~c~\text{end}~\{Post\}} $$

3. **Parallel Assignment Rule**
$$ \frac{}{\{Post[\overline{x}/\overline{e}]\}~\overline{x} := \overline{e}~\{Post\}} $$

4. **Conditional Rule**
$$ \frac{}{\{(b \implies wp(c_1)(Post)) \land (\neg b \implies wp(c_2)(Post))\}~\text{if}~b~\text{then}~c_1~\text{else}~c_2~\{Post\}} $$

### Operational Semantics Rules

1. **Conditional Execution**
$$ \frac{mem \vDash b}{\langle \text{if}~b~\text{then}~c_1~\text{else}~c_2, mem \rangle \rightarrow \langle c_1, mem \rangle} $$
$$ \frac{mem \not\vDash b}{\langle \text{if}~b~\text{then}~c_1~\text{else}~c_2, mem \rangle \rightarrow \langle c_2, mem \rangle} $$

2. **Assignment**
$$ \frac{}{\langle x := e, mem \rangle \rightarrow \langle \text{Done}, mem[x \mapsto \llbracket e \rrbracket(mem)] \rangle} $$
