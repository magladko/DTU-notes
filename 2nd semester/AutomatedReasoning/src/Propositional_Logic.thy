...
theorem
  \<open>p \<or> q \<longleftrightarrow> \<not> p \<longrightarrow> q\<close>
  \<open>p \<and> q \<longleftrightarrow> \<not> (p \<longrightarrow> \<not> q)\<close>
  by auto
...
theorem prover_correct: \<open>(\<forall>I. I \<Turnstile> p) \<longleftrightarrow> \<turnstile> p\<close>
...
proof (induct A B C D arbitrary: X Y rule: mp.induct[case_names (* Neg_L Neg_R *) Imp_L Imp_R Basic])
(*  case (Neg_L A B p C)
...
corollary \<open>(\<forall>I. I \<Turnstile> p) \<longleftrightarrow> [] \<then> [p]\<close>
  using OK[of \<open>[]\<close> \<open>[p]\<close>] by simp
